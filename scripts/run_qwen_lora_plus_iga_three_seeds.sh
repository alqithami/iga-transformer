...
for SEED in $SEEDS; do
  ...
  python -m iga_llm.train --config "$QWEN_CFG" --train_jsonl "$BASE_OUT/data/calibration_train_mix.jsonl" --dev_jsonl "$BASE_OUT/data/calibration_dev_mix.jsonl" --output_dir "$OUT/run" --seed "$SEED" --epochs 1 --lora_adapter_dir "$ADAPTER" | tee "logs/qwen_lora_plus_iga_seed${SEED}_train_$(date +%Y%m%d_%H%M%S).log"
  CKPT="$OUT/run/iga_modules.pt"
  for BENCH in truthfulqa fever halueval; do
    python -m iga_llm.evaluate --config "$QWEN_CFG" --data "$BASE_OUT/data/${BENCH}_eval.jsonl" --out "$OUT/predictions/${RUN_NAME}_${BENCH}_lora_iga_mc.jsonl" --method iga_mc --report_method lora_iga_mc --seed "$SEED" --run_id "$RUN_NAME" --iga_checkpoint "$CKPT" --lora_adapter_dir "$ADAPTER" --max_new_tokens 32 --num_samples 5 --temperature 0.7 --top_p 0.95
  done
